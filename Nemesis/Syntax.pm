...
use Sub::Exporter -setup => {
    ...
    installer => sub {
        my ( $args, $to_export ) = @_;
        my $pack = $args->{into};
        my $ctx_class = $args->{parser} || __PACKAGE__;
        ...
        foreach my $name (@$to_export) {
            if ( my $parser = __PACKAGE__->can("_parse_${name}") ) {
                Devel::Declare->setup_for(
                    $pack,
                    {   $name => {
                            const => sub { $ctx_class->$parser( $pack, @_ ) }
                        }
                    },
                );
            }
...
*debug_linestr =
    ...
    warn "  Substr: ",
        Devel::PartialDump::dump( substr( $ctx->get_linestr, $ctx->offset ) ),
