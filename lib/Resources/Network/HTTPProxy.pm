package Resources::Network::HTTPProxy;
### this is the HTTP::Proxy distribution, all credits go to the author
...
sub known_methods {
    ...
    exists $METHODS{$_} || carp "Method group $_ doesn't exist" for @args;
    my %seen;
...
sub start {
    ...
    while ( $self->loop ) {
        ...
        last
            ...
            && $self->conn >= $self->max_connections;
    }
...
sub serve_connections {
    ...
    $self->log( SOCKET, "SOCKET",
        "New connection from " . $conn->peerhost . ":" . $conn->peerport );
    ...
    SEND:
        ...
        $self->log( SOCKET, "SOCKET", "Connection closed by the proxy" ),
            last
    ...
    while ( my @ready = $select->can_read ) {
        for (@ready) {
            ...
            if ( $read == 0 ) {
                ...
                $self->log( SOCKET, "CONNECT",
                    "Connection closed by the $from" );
                $self->log( PROCESS, "PROCESS", "Served $served requests" );
                next;
            }
