...
sub encrypt() { #Take an outside public certs and encrypt for that destination the message
	my $self=shift;
	my $Public=shift;
	my $Message=shift;
	my $rsa = new Alt::Crypt::RSA::BigInt; 
...
sub sign()
{
	...
	my $Message=shift;
	my $rsa = new Alt::Crypt::RSA::BigInt; 
	my $Signature= $rsa->sign ( Message => $Message, Key => $self->PrivateKey ) || ($Init->io->error($rsa->errstr()) and return ());
    return $Signature;
}
...
sub verify(){
	...
	my $PubKey;
	my $rsa = new Alt::Crypt::RSA::BigInt; 
    my $Verify=$rsa->verify( Message => $Message, Signature => $Signature,
                  Key => $PubKey ) || ($Init->io->error($rsa->errstr()) and return ());
   return $Verify ;
}
...
sub connect() {
	...
	my $socket = IO::Socket::INET->new(
		...
		Proto=>$self->Proto, 
		Timeout => 3
	    ) || ($Init->io->error("Error connecting to ".$self->Node->ip.":".$self->Port." (".$self->Proto.")") and return ());
	$self->Socket($socket);
	$Init->io->debug("Connected to ".$self->Node->ip.":".$self->Port." (".$self->Proto.")",__PACKAGE__);
	return $socket;
}
...
sub listen(){
	...
	my $socket = IO::Socket::INET->new(
                Proto=>$self->Proto,             # protocol
           		LocalPort=>$self->Port,
                Reuse     => 1
	    ) || ($Init->io->error("Cannot bind to port ".$self->Port) and return ());
...
sub receive() {
	...
	while ($client = $self->Socket->accept() ) {     # receive a request
        $Init->io->debug("Connected from: ", $client->peerhost(),__PACKAGE__);  
