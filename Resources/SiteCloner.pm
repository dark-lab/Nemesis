package Resources::SiteCloner;
{
    ...
    get '/' => sub {
        ...
        say Dumper $self->tx->req->content->headers->host
            ; #$self->tx->req->content->headers->host contiene la richiesta dell'host remoto (ovvero che cosa ha digitato per raggiungerti)
        ...
        $self->render( text => $self->ua->get("http://www.perl.org") )
            ; #NB: Sbagliato Il suo output è : Mojo::Transaction::HTTP=HASH( _AREA_MEMORIA )
        ...
        $self->render(
            text => $self->ua->get("http://www.perl.org")->res->body );
    };
    ...
    get '/facebook' => sub {
        ...
        $mech->agent_alias('Windows IE 6')
            ; #Setto l'agent del client (ovviamente per facebook IE6 si vede da cazzo, dunque l'output non sarà spendido, ecco)
        $mech->get("http://www.facebook.it");    #Prendo il sito
        say $mech->res->decoded_content()
            ; #$mech->res ritorna un HTTP::Response object, che ha un metodo content e decode_content(content già decodificato, per comodità) per visualizzare il contenuto della risposta http
    ...
    any '/a' => sub {    #Esatto, gestisce anche tutto! :D
        ...
        $self->render(
            text => $self->ua->get(
                "http://" . $self->tx->req->content->headers->host
            )
        );               #Questo in teoria dovrebbe fare il nostro giochetto.
