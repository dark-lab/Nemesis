...
package Resources::Network::HTTPInterface;
use strict;
use warnings;
...
BEGIN {
    @attributes
        = qw(cookie_jar default_headers local_address max_redirect max_size proxy no_proxy timeout SSL_options verify_SSL);
    no strict 'refs';
...
sub post_form {
    ...
    ( @_ == 3 || @_ == 4 && ref $args eq 'HASH' )
        or Carp::croak(
        q/Usage: $http->post_form(URL, DATAREF, [HASHREF])/ . "\n" );
...
sub www_form_urlencode {
    ...
    while (@params) {
        ...
        if ( ref $value eq 'ARRAY' ) {
            ...
            push @terms,
                join( "=", map { $self->_uri_escape($_) } $key, $value );
        }
    }
...
sub _request {
    ...
    my ( $scheme, $host, $port, $path_query, $auth )
        = $self->_split_url($url);
...
sub read_header_lines {
    ...
    while () {
        ...
        if ( ++$lines >= $self->{max_header_lines} ) {
            ...
            $line =~ /\A ([^\x00-\x1F\x7F:]+) : [\x09\x20]* ([^\x0D\x0A]*)/x )
        {
            my ($field_name) = lc $1;
...
sub _find_CA_file {
    ...
    return $self->{SSL_options}->{SSL_ca_file}
        ...
        and -e $self->{SSL_options}->{SSL_ca_file};
