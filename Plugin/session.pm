...
my @PUBLIC_FUNCTIONS = qw(list wrap spawn stash);    #NECESSARY
...
sub spawn()
{
	...
	$self->{'core'}->{'Session'} =
		$self->{'core'}->{'ModuleLoader'}->loadmodule("Session");
...
{
	my $self = shift;
	$self->{'core'}->{'Session'}->wrap();
}

sub stash{
	#Give your session to the dogs
	my $self=shift;
	$self->{'core'}->{'Session'}->stash();
...
sub save()
{
	...
	my $Session = $self->{'core'}->{'Session'};
	$Session->save();
	$self->{'core'}->{'IO'}->print_info("session saved.");
}
...
sub save_state()
{
	...
	$self->{'core'}->{'IO'}->print_info( "Session saved with name: "
							 . $Session->{'CONF'}->{'VARS'}->{'SESSION_NAME'} );
	$Session->save();
}
...
sub restore($)
{
	...
	$Session->restore($module);
	$self->{'core'}->{'IO'}->debug(
				"ModuleName:" . $Session->{'CONF'}->{'VARS'}->{'MODULE_NAME'} );
	$self->{'core'}->{'IO'}->debug( "Moduli attuali:"
				   . Dumper( $self->{'core'}->{'ModuleLoader'}->{'modules'} ) );
